<x-layout title="Lists I Follow — FLiK">
    <div class="min-h-screen bg-black pt-20 pb-16">
        <div class="container mx-auto px-4 md:px-8 lg:px-16 max-w-[1400px]">

            <div class="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-8">
                <div>
                    <h1 class="font-heading text-2xl md:text-4xl font-bold" style="color: #C5A55A">Following</h1>
                    <p class="mt-1 text-sm text-gray-400">List yang kamu ikuti dari kurator lain.</p>
                </div>
            ...
            @if($lists->total() === 0)
                <div class="flex flex-col items-center justify-center py-24 text-center rounded-xl"
                    ...
                    <x-icon name="heart" :size="36" class="text-gray-700 mb-3" />
                    <h2 class="text-lg font-heading font-semibold text-gray-400">Belum mengikuti list apa pun</h2>
                    <p class="text-sm text-gray-600 mt-1">Jelajahi list yang sedang populer.</p>
                    <a href="{{ route('user-lists.index') }}"
                       ...
                       style="background: linear-gradient(135deg, #C5A55A, #E8D5A3);">
                        <x-icon name="search" :size="14" /> Discover Lists
                    </a>
                </div>
            @else
