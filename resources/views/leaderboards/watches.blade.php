        ...
        <div class="container mx-auto px-4 md:px-16 max-w-4xl">
            ...
            <div class="mb-6 flex flex-wrap gap-2">
                ...
                <a href="{{ route('leaderboards.watches') }}"
                   ...
                   style="background: linear-gradient(135deg, #C5A55A, #E8D5A3)">🎬 Watches</a>
            </div>

            <h1 class="font-heading text-2xl md:text-3xl font-bold text-white mb-2">Most Watched</h1>
            <p class="text-sm text-gray-400 mb-6">Top 50 user dengan jumlah film selesai terbanyak.</p>
                ...
                <div class="mb-6 p-4 rounded-xl flex items-center gap-6"
                    ...
                    <div class="flex-1">
                        ...
                        <div class="font-heading text-xl font-bold text-white">
                            #{{ $myRank }}
                            <span class="text-sm font-normal text-gray-400 ml-2">— {{ $myCount }} film selesai</span>
                        </div>
                    </div>
                </div>
            @endif
            ...
            @if($top->isEmpty())
                <div class="text-center py-20 rounded-xl" style="background: rgba(20,18,16,0.5); border: 1px solid rgba(197,165,90,0.15)">
                    <p class="text-sm text-gray-500">Belum ada film yang selesai ditonton.</p>
                </div>
            @else
                    ...
                    @foreach($top as $i => $row)
                        ...
                        <div class="flex items-center gap-4 px-4 py-3 {{ !$loop->last ? 'border-b border-gray-800/50' : '' }} {{ $isMe ? 'bg-[#C5A55A]/5' : '' }}">
                            ...
                            <div class="text-right">
                                <div class="text-lg font-bold text-white">🎬 {{ number_format($row->watch_count) }}</div>
                                <div class="text-[10px] text-gray-500">film selesai</div>
                            </div>
                        </div>
                    @endforeach
                </div>
